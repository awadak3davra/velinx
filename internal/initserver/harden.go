...
// key (base64) so wakeroute can hand it back for the user to download. It does NOT touch
...
cp "$SSHD" "$SSHD.wakeroute.bak" 2>/dev/null || true
...
if [ -d /etc/ssh/sshd_config.d ]; then
  printf 'PasswordAuthentication no\nPubkeyAuthentication yes\n' > /etc/ssh/sshd_config.d/00-wakeroute-hardening.conf
fi
