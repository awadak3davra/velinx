...
// cutover.go is the device-mutating cutover from the keen-pbr + S89 stack to WakeRoute. It is
...
// oldStackServices are the keen-pbr + failover init.d services WakeRoute replaces, in boot/
...
// main-table default, so it keeps working alongside WakeRoute's default→wr-tun; it is re-
// applied by 60-wgbot-policy.sh (which WakeRoute also keeps), NOT the keen-pbr netfilter hook.
// Replicating its 8589-CIDR ipset/table would be needless risk. Also kept: sockd, DNS chain,
// nfqws, cams_isolate. WakeRoute's kernel plane takes over S87's bypasses/mgmt-reverse/default.
...
// can't fight WakeRoute's routing.
...
// Cutover performs the device-mutating cutover from the keen-pbr + S89 stack to WakeRoute,
