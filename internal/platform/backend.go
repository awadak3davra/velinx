...
// RoutingBackend applies a WakeRoute profile's routing to the host and removes it. There is
// one implementation per platform — OpenWrt (nft/fw4 PBR + sing-box) and Keenetic (native
// NDM interfaces/routes + a sing-box fallback for non-native protocols). cmd/wakeroute
...
// (it replaces the prior WakeRoute state) — and must be safe to call repeatedly.
type RoutingBackend interface {
	// Platform reports which platform this backend drives.
	Platform() Platform
	// Apply realizes the profile's routing, replacing any previously-applied WakeRoute
	...
	// Teardown removes all WakeRoute-managed routing this backend applied.
