...
func TestPass4_HybridApplyInstallsPlan(t *testing.T) {
	...
	if !rrStdinHasFrom(rr, 0, "198.51.100.0/24") || !rrStdinHasFrom(rr, 0, "delete table inet wakeroute_pbr") {
...
func TestPass4_RollbackRestoresBaseline(t *testing.T) {
	...
	if !rrHasFrom(rr, mark, "nft delete table inet wakeroute_pbr") {
...
// wakeroute_pbr table left by a prior hybrid era (the user switched mode + rebooted).
func TestPass4_BootSyncOrphanTeardown(t *testing.T) {
	s, rr := pbrApplyServer(t)
	s.cfg.RoutingMode = "tun"
	seedVoWiFi(t, s)
	s.SyncPlugins()
	if !rrHasFrom(rr, 0, "nft delete table inet wakeroute_pbr") {
