#!/bin/sh
# WakeRoute (wakeroute) uninstaller for Entware / Keenetic.
#
# Stops the service, then removes the binary, init script and the single rolling
# backup. By DEFAULT it KEEPS your config + runtime state (/opt/etc/wakeroute,
# /opt/var/wakeroute); pass --purge to delete those too.
#
...
# is already gone). Only WakeRoute's own paths are touched -- shared deps
...
INIT="$INITD/S99wakeroute"
BINARY="$SBIN/wakeroute"
BACKUP="$SBIN/wakeroute.bak"
...
usage() {
  cat <<'USAGE'
WakeRoute uninstaller for Entware / Keenetic.

Usage: sh ./uninstall.sh [options]

Options:
      --purge        also delete config + runtime state (/opt/etc/wakeroute,
                     /opt/var/wakeroute) -- this removes your saved connections
...
say "WakeRoute uninstaller (Entware / Keenetic)"
...
if [ -e "$SBIN/wakeroute.new" ]; then
  rm -f "$SBIN/wakeroute.new" && ok "removed $SBIN/wakeroute.new (stale staging file)"
fi
[ -z "$REMOVED" ] && info "nothing to remove (WakeRoute binary/service not present)"
...
hdr "Done"
if [ "$PURGE" = 1 ]; then
  say "WakeRoute fully removed (binary, service, config + data)."
else
  say "WakeRoute binary + service removed; config kept (use --purge to delete it)."
fi
